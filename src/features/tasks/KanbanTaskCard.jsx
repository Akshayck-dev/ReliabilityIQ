...
const KanbanTaskCard = ({ task, role }) => {
    ...
    return (
        <div
            ...
            {...listeners}
            {...attributes}
            className={`group bg-white dark:bg-slate-800 rounded-xl p-4 shadow-sm border border-slate-200 dark:border-slate-700 transition-all duration-200 hover:shadow-md hover:border-slate-300 dark:hover:border-slate-600 ${isDragging ? 'shadow-xl ring-2 ring-orange-400/40 scale-[1.02]' : ''}`}
        >
            {/* Title */}
            <Link
                to={`/tasks/${task.id}`}
                className="text-sm font-semibold text-slate-900 dark:text-slate-100 hover:text-[#ea580c] dark:hover:text-orange-400 transition-colors line-clamp-2 block"
